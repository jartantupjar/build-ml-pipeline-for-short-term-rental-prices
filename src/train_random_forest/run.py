...
def go(args):
    ...
    # save the returned path in train_local_path
    ...
    # Save the sk_pipe pipeline in the directory "random_forest_dir"
    mlflow.sklearn.save_model(sk_pipe,
                              "random_forest_dir",
                              input_example=X_val[:10])

    # Create an artifact and Upload the model we just exported to W&B
    artifact = wandb.Artifact(
        args.output_artifact,
        type="model_export",
        description="this is a trained random forest model for predicting rental prices",
        metadata=rf_config)
    ...
    # Here we save r_squared under the "r2" key and log the variable "mae"
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # with the random forest instance that we just saved in the
    # `random_forest` variable.
    sk_pipe = Pipeline(
        steps=[
